...
if ($dayOfMonth >= 1 && $dayOfMonth <= 15) {
	...
	if ($emptype == 'Permanent'){
		...
		/** PAG-IBIG LOAN DEDUCTIONS **/
			$pagibigloanquery = "SELECT emp_id,monthly_deduct,no_of_pays, loan_balance, loanstart_date, loanend_date FROM LOANpagibig WHERE emp_id = '$payID' AND MONTH('$payperiodfrom') BETWEEN MONTH(loanstart_date) AND MONTH(loanend_date) AND status ='On-Going'";
			$pagibigloanexecqry = mysqli_query($conn,$pagibigloanquery) or die ("FAILED TO CHECK PAG-IBIG LOANS12" . mysqli_error($conn)); ;
			...
			if ($pagibigloanarray){
				...
				if($pagibignoofpays>0){
					...
					$pagibigloanupdateqry = "UPDATE LOANpagibig SET no_of_pays = '$pagibigremain', loan_balance = '$pagibigbalance' WHERE emp_id = '$payID' AND MONTH('$payperiodfrom') BETWEEN MONTH(loanstart_date) AND MONTH(loanend_date)";
					$pagibigloanupdateexecqry = mysqli_query($conn,$pagibigloanupdateqry) or die ("FAILED TO CHECK PAGIBIG LOANS3 ".mysqli_error($conn));
					if ($pagibigremain == 0){
						$updatepagibigloan = "UPDATE LOANpagibig SET status = 'Paid' WHERE emp_id = '$payID' AND MONTH('$payperiodfrom') BETWEEN MONTH(loanstart_date) AND MONTH(loanend_date)";
						$updatepagibigloanexecqry = mysqli_query($conn,$updatepagibigloan) or die ("FAILED TO CHECK PAG-IBIG LOANS4");

						$notificationMessage2 = "PAGIBIG Loan has been paid.";
						$insertNotificationQuery2 = "INSERT INTO empnotifications (admin_id,emp_id, message, type, status) VALUES ('$adminId', '$payID','$notificationMessage2','Loan','unread')";
						mysqli_query($conn, $insertNotificationQuery2);
						}
		...
		$gsisloanquery = "SELECT emp_id,monthly_deduct,no_of_pays, loan_balance, start_date, end_date FROM LOANgsis WHERE emp_id = '$payID' AND MONTH('$payperiodfrom') BETWEEN MONTH(start_date) AND MONTH(end_date) AND status ='On-Going'";
		...
		if ($gsisloanarray){
			...
			$gsismonthlyloan = $gsisloanarray['monthly_deduct'];
				...
				if ($gsisnoofpays>0){
					...
					$gsisloanupdateqry = "UPDATE LOANgsis SET no_of_pays = '$gsisremain', loan_balance = '$gsisbalance' WHERE emp_id = '$payID' AND MONTH('$payperiodfrom') BETWEEN MONTH(start_date) AND MONTH(end_date)";
					$gsisloanupdateexecqry = mysqli_query($conn,$gsisloanupdateqry) or die ("FAILED TO UPDATE gsis LOAN RECORD1 ".mysqli_error($conn)); 
					if ($gsisremain == 0){
					$updategsisloan = "UPDATE LOANgsis SET status = 'Paid'  WHERE emp_id = '$payID' AND MONTH('$payperiodfrom') BETWEEN MONTH(start_date) AND MONTH(end_date)";
					$updategsisloanexecqry = mysqli_query($conn,$updategsisloan) or die ("FAILED TO CHECK PAG-IBIG LOANS1" .mysqli_error($conn)); 

					$notificationMessage1 = "GSIS Loan has been paid.";
					$insertNotificationQuery1 = "INSERT INTO empnotifications (admin_id,emp_id, message, type, status) VALUES ('$adminId', '$payID','$notificationMessage1','Loan','unread')";
					mysqli_query($conn, $insertNotificationQuery1);
					}
				} else {

					$updategsisloan = "UPDATE LOANgsis SET status = 'Paid'  WHERE emp_id = '$payID' AND MONTH('$payperiodfrom') BETWEEN MONTH(start_date) AND MONTH(end_date)";
					$updategsisloanexecqry = mysqli_query($conn,$updategsisloan) or die ("FAILED TO CHECK PAG-IBIG LOANS2" .mysqli_error($conn)); 
	...
	/** TOTAL DEDUCTIONS COMPUTATION **/
	$savepayrollquery = "INSERT INTO PAY_PER_PERIOD (emp_id,pperiod_range,pperiod_month,pperiod_year,rate_per_hour,reg_pay, ot_pay, otrh_pay,otsh_pay, hday_pay,shday_pay, sss_deduct, sssloan_deduct, pagibigloan_deduct, total_deduct,lv_pay, undertimehours, late) VALUES 
												('$payID','$payperiodrange','$month','$year','$rph','$basepay','$ot','$otrh','$otsh','$RHpay','$SHpay','$gsis', '$gsismonthlyloan','$pagibigmonthlyloan','$totaldeduct','$LVpay', '$undertimehours','$late')";
	$savepayrollexecquery = mysqli_query($conn,$savepayrollquery) or die ("FAILED TO INSERT PAYROLL INFO ".mysqli_error($conn));
